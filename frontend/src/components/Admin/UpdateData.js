...
const UpdateData = () => {
    ...
    const fetchData = () => {
        return (
            axios.get("https://api.bcapoints.in:8000:8000:8000/api/posts").then((response) => setData(response.data))
        )
    }

    const deleteData =  (id) =>{
        const yes = window.confirm("Do You Really Want to Delete?");
         yes && axios.delete(`https://api.bcapoints.in:8000:8000:8000/api/posts/${id}`).then(()=> alert("deleted Successfully") )
   }
